...
""" 
we first analyze if the user has verify their identity via slack first , if the user hasn't
we will sent a verification code through slack and the user will have a token that we will check for.

The token lasts 15 min and allows the user to add/edit or delete any route, this procedure will be the same for 
the add/edit/delete functions 
...
"""
whenever we add/edit or delete a route we need to first check who is the user performing the action and what institution 
does the user belongs to, this check is necessary so nobody perfoms an action in a network is not theirs.
...
""" 
the difference between the delete method and 'exterminate' function is that delete does not eliminate the route, 
it leaves the route 'INACTIVE' (save on the db but not on the router) meanwhile exterminate eliminates the route 
from both the DB and the router.
"""
...
""" 
Method that makes an asynchronous request to the server when a graph is needed or asked for.
"""
